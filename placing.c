...
/*   Updated: 2020/02/01 15:47:00 by zszeredi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_letter(t_table *s2, t_tetra *s, int nb, t_add *p)
{
	...
	while (counter < 4)
	{
		...
		{
			s2->square[b + p->add2][a + p->add] = s->letter;
			counter++;
		}
		else
			return (-1);
	...
	while (p->counter < 4)
	{
		if (p->b + p->add2 < s2->table_size)
		{
			...
			if (p->a + p->add + p->move < s2->table_size)
			{
				...
				within_table(s2, p);
				s2->square[p->b + p->add2][p->a + p->add + p->move] == '.' ?
					last_move(p, p->move) : restart(p);
			}
			else
				new_line(p);
		}
...
int		place(t_table *s2, t_tetra *s, int nb) // for some reason does not go back to backtracking via the return in case of multiple tetros
{
	...
	if (nb == 0)
	{
		if ((ft_letter(s2, s, nb, p)) == 0)//letter might has to be incremented elswhere
			tmp = tempo(s2);
		return (1);
	}
	else// if we are not at the first one
	{
		while (s->letter++ && (ft_compare(s2, s, nb, m)) < 0 && nb > 0)
		{
